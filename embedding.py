...
def build_item_prompt(row):
    base_template = """内容类型为{type}，发布于{pub_time}，{status}被推荐。
    内容的标题是{title}，一级标签为{category}。作者身份为{author_status}，所属话题为{theme_id}，活动id为{activity}，发布源为{publish}。
...
def generate_user_embs(
    ...
    with amp_ctx:
        for batch in tqdm(dataloader, desc="Generating User Embeddings"):
            ...
            messages_list = [
                [
                    {"role": "system", "content": "个性化内容推荐场景中，根据用户信息，生成个性化内容推荐。"},
